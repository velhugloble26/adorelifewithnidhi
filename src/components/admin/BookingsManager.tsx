...
import { ADMIN_AVAILABILITY, ADMIN_BOOKING, ADMIN_BOOKING_CANCEL, ADMIN_BOOKING_RESCHEDULE, ADMIN_BOOKING_STATUS, ADMIN_BOOKINGS } from "@/utils/api";
...
type Booking = { bookingId: string; firstName: string; lastName: string; email: string; phone: string; whatsappNumber: string; identifyYourGender?: string; dob?: string; location?: string; sessionMode?: string; occupation?: string; relationShipStatus?: string; numberOfChildren?: number; currentlyTakingAnyPsychiatricMedication?: boolean; medicationDetails?: string; whereuknowaboutus?: string; therapyGoals?: string[]; therapyGoalsOther?: string; addNotes?: string; InformedConsentforTherapySessions?: string; conformationOfBooking?: boolean; packageId: string; packageName: string; packagePrice: number; selectedDate: string; selectedTime: string; sessionType: string; paymentMethod: string; paymentStatus: string; bookingStatus: string; created_at?: string };
const statuses = ["pending", "confirmed", "completed", "cancelled", "no_show"];
const therapyGoalOptions = ["Managing stress, anxiety, or overwhelming emotions", "Healing from past trauma or unresolved emotional pain", "Improving self-confidence and self-esteem", "Navigating relationship challenges (family, partner, friends, etc.)", "Coping with grief or loss", "Developing healthier coping mechanisms and habits", "Enhancing communication and interpersonal skills", "Gaining clarity and direction in life", "Overcoming workplace or career-related challenges", "Achieving emotional balance and inner peace", "Other"];
...
export default function BookingsManager() {
  ...
  const [filters, setFilters] = useState({ date: "", status: "", sessionType: "", paymentStatus: "", packageId: "" }); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [success, setSuccess] = useState(""); const [editing, setEditing] = useState<Booking | null>(null); const [viewing, setViewing] = useState<Booking | null>(null); const [acting, setActing] = useState("");
  ...
  return <>
    ...
    {viewing && <BookingDetailsForm booking={viewing} onClose={() => setViewing(null)} onSaved={(message, updated) => { setViewing(null); setSuccess(message); setItems((current) => current.map((item) => item.bookingId === updated.bookingId ? updated : item)); }} />}
  </>;
}

function BookingDetailsForm({ booking, onClose, onSaved }: { booking: Booking; onClose: () => void; onSaved: (message: string, booking: Booking) => void }) {
  const [form, setForm] = useState({ ...booking, therapyGoals: booking.therapyGoals || [] });
  const [busy, setBusy] = useState(false); const [error, setError] = useState("");
  const update = (field: string, value: unknown) => setForm((current) => ({ ...current, [field]: value }));
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { const formValues = form as unknown as Record<string, unknown>; const payload = await requestApi(ADMIN_BOOKING(booking.bookingId), { method: "PATCH", body: JSON.stringify(Object.fromEntries(Object.entries(formValues).filter(([, value]) => value !== undefined && value !== ""))) }); onSaved(payload.message, payload.data.booking); } catch (err) { setError(err instanceof Error ? err.message : "Unable to update booking."); } finally { setBusy(false); } }
  return <Modal title={`Booking details: ${booking.bookingId}`} onClose={onClose}><form onSubmit={submit} className="space-y-5 max-h-[75vh] overflow-y-auto pr-2">{error && <Notice>{error}</Notice>}<div className="grid sm:grid-cols-2 gap-4">{([["firstName", "First name"], ["lastName", "Last name"], ["email", "Email"], ["phone", "Phone"], ["whatsappNumber", "WhatsApp"], ["dob", "Date of birth"], ["location", "Location"], ["occupation", "Occupation"], ["medicationDetails", "Medication details"], ["therapyGoalsOther", "Other therapy goal"], ["addNotes", "Additional notes"]] as const).map(([field, label]) => <label key={field} className="admin-label">{label}<input className={fieldClass} value={String((form as unknown as Record<string, unknown>)[field] || "")} onChange={(e) => update(field, e.target.value)} /></label>)}<label className="admin-label">Gender<select className={fieldClass} value={form.identifyYourGender || ""} onChange={(e) => update("identifyYourGender", e.target.value)}><option value="">Not provided</option><option>Male</option><option>Female</option><option>Non-Binary</option><option>Transgender</option><option>Prefer not to say</option><option>Other</option></select></label><label className="admin-label">Session mode<select className={fieldClass} value={form.sessionMode || ""} onChange={(e) => update("sessionMode", e.target.value)}><option value="">Not provided</option><option>Online</option><option>Offline</option></select></label><label className="admin-label">Relationship status<select className={fieldClass} value={form.relationShipStatus || ""} onChange={(e) => update("relationShipStatus", e.target.value)}><option value="">Not provided</option><option>Single</option><option>In a relationship</option><option>Married</option><option>Divorced</option><option>Widowed</option><option>Other</option></select></label><label className="admin-label">Number of children<input className={fieldClass} type="number" min="0" value={form.numberOfChildren ?? 0} onChange={(e) => update("numberOfChildren", Number(e.target.value))} /></label><label className="admin-label">How they heard about us<select className={fieldClass} value={form.whereuknowaboutus || ""} onChange={(e) => update("whereuknowaboutus", e.target.value)}><option value="">Not provided</option><option>Social Media</option><option>Friend/Family</option><option>Search Engine</option><option>Advertisement</option><option>Other</option></select></label></div><div className="space-y-3 text-sm"><label className="flex gap-2"><input type="checkbox" checked={Boolean(form.currentlyTakingAnyPsychiatricMedication)} onChange={(e) => update("currentlyTakingAnyPsychiatricMedication", e.target.checked)} /> Taking psychiatric medication</label><label className="flex gap-2"><input type="checkbox" checked={Boolean(form.InformedConsentforTherapySessions)} onChange={(e) => update("InformedConsentforTherapySessions", e.target.checked ? "I agree" : "")} /> Consent given</label><label className="flex gap-2"><input type="checkbox" checked={Boolean(form.conformationOfBooking)} onChange={(e) => update("conformationOfBooking", e.target.checked)} /> Booking confirmed by client</label></div><div><strong className="text-sm">Therapy goals</strong><div className="mt-2 grid gap-2 text-sm">{therapyGoalOptions.map((goal) => <label key={goal} className="flex gap-2"><input type="checkbox" checked={form.therapyGoals?.includes(goal)} onChange={(e) => update("therapyGoals", e.target.checked ? [...(form.therapyGoals || []), goal] : (form.therapyGoals || []).filter((item) => item !== goal))} /> {goal}</label>)}</div></div><div className="flex justify-end gap-3"><button type="button" className="admin-button-secondary" onClick={onClose}>Cancel</button><button className="btn-primary" disabled={busy}>{busy ? "Saving…" : "Save details"}</button></div></form></Modal>;
}
