...
import { GET_ALL_CONTACTS, GET_ALL_QUOTES } from "@/utils/api";
...
export default function EnquiriesManager() {
  ...
  const load = useCallback(async () => {
    ...
    try { const query = new URLSearchParams({ page: String(page), limit: "20" }); if (search) query.set("search", search); const url = tab === "contacts" ? GET_ALL_CONTACTS : GET_ALL_QUOTES; const payload = await requestApi(`${url}?${query}`); setItems(payload.data?.data || []); setPagination(payload.data?.pagination); }
  ...
  return <>
    ...
    {loading ? <LoadingState label="Loading enquiries…" /> : items.length === 0 ? <EmptyState title="No enquiries found" body={search ? "Try a different search." : "New submissions will appear here."} /> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Name</th><th>Phone / WhatsApp</th><th>Email Address</th><th>Preferred Session</th><th>Preferred Format</th><th>Message</th><th>Created At / Submission Date</th></tr></thead><tbody>{items.map((item) => <tr key={item._id}><td><strong>{item.name || "—"}</strong></td><td>{item.phone || "—"}</td><td>{item.email || "—"}</td><td>{item.preferredSession || item.selectedOption || "—"}</td><td>{item.preferredFormat || "—"}</td><td className="min-w-[260px]"><p className="whitespace-pre-wrap break-words">{item.message || "—"}</p></td><td className="min-w-[180px]">{item.created_at ? new Date(item.created_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</td></tr>)}</tbody></table></div>}
